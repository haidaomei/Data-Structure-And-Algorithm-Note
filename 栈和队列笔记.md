...
队列是一种先进先出(First In First Out,FIFO)的结构,规定front索引指向队首,rear索引指向队尾下一个单位,程序如下
...
栈和队列的生长方向是人为规定的,上述都以高地址为生长方向

同时栈顶的top指向栈顶的下一个地址单位还是直接指向栈顶都要根据实际情况判断

如果初始top指针超出了数组索引,基本可以判断是top指向栈顶元素,反之则否

如果top--作为入栈,则是向低地址生长的栈,反之则高
