...
上面使用了栈,也可以使用队列,如果使用栈,那查找下一个度数为0的节点就是往深查找,反之往旁边查找,具体到程序是while中的if,假设现有根节点k1,去除k1之后两个子节点k2、k3度数皆为0,则栈顶为k3,则下一个出栈的一定是k3,那么入栈的就是k3的子节点,而不是把k2、k3都出队了再处理k2、k3的下一层,这就是和队列的区别
